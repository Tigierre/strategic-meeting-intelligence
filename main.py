...
async def research_with_perplexity(query, language='it'):
   """Research external context using Perplexity API"""
   try:
       perplexity_key = os.environ.get('PERPLEXITY_API_KEY')
       if not perplexity_key:
           return {'success': False, 'error': 'Perplexity API key not found'}
       
       # Construct research query based on language
       if language == 'it':
           research_query = f"""
           Cerca informazioni recenti (ultimi 12 mesi) su: {query}
           Includi:
           - Trend di mercato correlati
           - Benchmark del settore
           - Opportunità competitive
           - Sviluppi tecnologici rilevanti
           Rispondi in italiano con dati concreti e fonti.
           """
       else:
           research_query = f"""
           Find recent information (last 12 months) about: {query}
           Include:
           - Related market trends
           - Industry benchmarks  
           - Competitive opportunities
           - Relevant technological developments
           Respond with concrete data and sources.
           """
           ...
           response = await client.post(
               ...
               json={
                   ...
                   "messages": [
                       {
                           "role": "system", 
                           "content": "You are a strategic business researcher. Provide factual, recent information with sources."
                       },
                       {
                           "role": "user", 
       ...
       if response.status_code == 200:
           ...
           return {'success': True, 'research': content}
       else:
           return {'success': False, 'error': f'Perplexity API error: {response.status_code}'}
           
   except Exception as e:
       return {'success': False, 'error': f'Research failed: {str(e)}'}

def extract_research_topics(analysis, language='it'):
   """Extract key topics for external research"""
   topics = []
   
   # Extract from strategic insights
   if 'strategic_insights' in analysis:
       for insight in analysis['strategic_insights'][:2]:  # Top 2 insights
           topic = insight.get('insight', '')
           if len(topic) > 20:  # Valid topic
               topics.append(topic[:100])  # Truncate for API
   
   # Extract from innovation opportunities  
   if 'innovation_opportunities' in analysis:
       for opp in analysis['innovation_opportunities'][:2]:  # Top 2 opportunities
           topic = opp.get('opportunita', '')
           if len(topic) > 20:
               topics.append(topic[:100])
   
   # Extract from recurring themes with high importance
   if 'recurring_themes' in analysis:
       for theme in analysis['recurring_themes']:
           if theme.get('importanza') == 'alta':
               topic = theme.get('tema', '')
               if len(topic) > 10:
                   topics.append(topic[:100])
   
   return topics[:3]  # Max 3 topics to research

async def enhance_analysis_with_research(analysis, language='it'):
   """Enhance analysis with external research"""
   try:
       # Extract topics to research
       topics = extract_research_topics(analysis, language)
       
       if not topics:
           return analysis  # Return original if no topics
       
       # Research each topic
       research_results = []
       for topic in topics:
           research = await research_with_perplexity(topic, language)
           if research['success']:
       ...
       if research_results:
           analysis['external_intelligence'] = research_results
       
       return analysis
       
   except Exception as e:
       # Return original analysis if research fails
       return analysis

def run_async_research(analysis, language):
   """Wrapper to run async research in sync context"""
   try:
       loop = asyncio.new_event_loop()
       asyncio.set_event_loop(loop)
       result = loop.run_until_complete(enhance_analysis_with_research(analysis, language))
       loop.close()
       return result
   except Exception as e:
       # Return original analysis if async fails
       return analysis
...
def main():
   ...
   if st.sidebar.button("🔍 Debug API Keys"):
       ...
       perplexity_key = os.environ.get('PERPLEXITY_API_KEY', 'NOT_FOUND')
       ...
       st.sidebar.write(f"Perplexity Key: {'✅ Found' if perplexity_key != 'NOT_FOUND' else '❌ Not found'}")
   ...
   if uploaded_file:
       ...
       use_external_research = st.sidebar.checkbox(
           "External Research", 
           value=True, 
           help="Enhance insights with Perplexity market research"
       )
           ...
           with st.spinner("🎙️ Processing audio file..."):
               ...
               if use_advanced_analysis:
                   ...
                   if ai_result['success']:
                       # Step 4: External Research Enhancement (if enabled)
                       if use_external_research:
                           status_text.text("🔍 Researching external context with Perplexity...")
                           progress_bar.progress(80)
                           
                           enhanced_analysis = run_async_research(
                               ai_result['analysis'], 
                               detected_language
                           )
                           ai_result['analysis'] = enhanced_analysis
                           
                           st.success("✅ External research completed!")
                       
                       lang_display = "🇮🇹 Italiano" if detected_language == 'it' else "🇬🇧 English"
                       research_suffix = " + External Research" if use_external_research else ""
                       st.success(f"✅ Strategic analysis{research_suffix} completed in {lang_display}!")
                   else:
...
def display_analysis_results(analysis):
   ...
   with col4:
       if analysis.get('external_research_enabled', False):
           if 'external_intelligence' in analysis.get('ai_analysis', {}):
               st.success("✅ External research completed")
           else:
               st.info("ℹ️ External research attempted")
       else:
           st.info("ℹ️ External research skipped")
   ...
   if analysis['ai_analysis']:
       ...
       # Create enhanced tabs - check if external research is available
       has_external_research = 'external_intelligence' in analysis['ai_analysis']
       ...
       # External Research tab (only if research was performed)
       if has_external_research:
           with tab8:
               if 'external_intelligence' in analysis['ai_analysis']:
                   st.info("🔍 Ricerca automatica eseguita su Perplexity per approfondire i temi principali del meeting")
                   
                   for i, research in enumerate(analysis['ai_analysis']['external_intelligence'], 1):
                       st.write(f"**🔍 {i}. Ricerca su: {research['topic'][:80]}...**")
                       
                       # Display research results in expandable section
                       with st.expander(f"📊 Risultati ricerca #{i}", expanded=False):
                           st.markdown(research['research'])
                       
                       st.divider()
               else:
                   st.info("🔍 La ricerca esterna è stata tentata ma non ha prodotto risultati. Verifica la configurazione dell'API Perplexity.")
